...
prop.table(table(df_hrs$mobility_status_hrs))
prop.table(table(df_elsa$mobility_status_elsa))
prop.table(table(df_mhas$mobility_status_mhas))
prop.table(table(df_lasi$mobility_status_lasi))
prop.table(table(df_charls$mobility_status_charls))
...
create_forest_plot_hrs <- function(model, title) {
  ...
  mobility_terms <- rownames(coef_table)[grep("^mobility_status_hrs", rownames(coef_table))]
  ...
  reference_data <- data.frame(
    Term = "Stably low (ref)",
    Estimate = 0, LowerCI = 0, UpperCI = 0, PValue = NA
  )
  
  forest_data <- rbind(reference_data, forest_data)
  
  forest_data$Label <- recode(forest_data$Term,
                              "mobility_status_hrsDownwardly mobile" = "Downwardly mobile",
                              "mobility_status_hrsStably middle" = "Stably middle",
                              "mobility_status_hrsUpwardly mobile" = "Upwardly mobile",
                              "mobility_status_hrsStably high" = "Stably high",
                              "Stably low (ref)" = "Stably low (ref)"  
  )
  
  forest_data$Label <- factor(forest_data$Label, levels = c("Stably low (ref)", "Downwardly mobile", "Stably middle", "Upwardly mobile", "Stably high"))
  ...
  ggplot(forest_data, aes(x = Label, y = Estimate)) +
    ...
    scale_y_continuous(breaks = c(0, 1, 2), labels = c("0", "1", "2"), limits = c(-0.5, 2)) +
    labs(title = title, y = "Coefficient Estimate (95% CI)", x = "") +
    theme_minimal() +
    theme(
...
####us####
model1_fgcp <- lm(fgcp_hrs ~ mobility_status_hrs + age_hrs + sex_hrs + Race + urbanicity_hrs,data = df_hrs)
hrs_fgcp <- create_forest_plot_hrs(model1_fgcp, " ")
print(hrs_fgcp)
summary(model1_fgcp)

model1_fexf <- lm(fexf_hrs ~ mobility_status_hrs + age_hrs + sex_hrs + Race + urbanicity_hrs, data = df_hrs)
hrs_fexf <- create_forest_plot_hrs(model1_fexf, " ")
print(hrs_fexf)

model1_flang <- lm(flang_hrs ~ mobility_status_hrs + age_hrs + sex_hrs + Race + urbanicity_hrs, data = df_hrs)
hrs_flang <- create_forest_plot_hrs(model1_flang, " ")
print(hrs_flang)

model1_forient <- lm(forient_hrs ~ mobility_status_hrs + age_hrs + sex_hrs + Race + urbanicity_hrs, data = df_hrs)
hrs_forient <- create_forest_plot_hrs(model1_forient, " ")
print(hrs_forient)

model1_fmem <- lm(fmem_hrs ~ mobility_status_hrs + age_hrs + sex_hrs + Race + urbanicity_hrs, data = df_hrs)
...
create_forest_plot_charls <- function(model, title) {
  ...
  mobility_terms <- rownames(coef_table)[grep("^mobility_status_charls", rownames(coef_table))]
  
  forest_data <- data.frame(
    Term = mobility_terms, Estimate = coef_table[mobility_terms, "Estimate"],
    LowerCI = conf_int[mobility_terms, 1], UpperCI = conf_int[mobility_terms, 2], PValue = coef_table[mobility_terms, "Pr(>|t|)"]
  )
  
  reference_data <- data.frame(Term = "Stably low (ref)", Estimate = 0, LowerCI = 0, UpperCI = 0, PValue = NA)
  forest_data <- rbind(reference_data, forest_data)
  
  forest_data$Label <- recode(forest_data$Term,
                              "mobility_status_charlsDownwardly mobile" = "Downwardly mobile",
                              "mobility_status_charlsStably middle" = "Stably middle",
                              "mobility_status_charlsUpwardly mobile" = "Upwardly mobile",
                              "mobility_status_charlsStably high" = "Stably high",
                              "Stably low (ref)" = "Stably low (ref)"  
  )
  forest_data$Label <- factor(forest_data$Label, levels = c("Stably low (ref)", "Downwardly mobile", "Stably middle", "Upwardly mobile", "Stably high"))
...
##china#
model1_fgcp <- lm(fgcp_charls ~ mobility_status_charls + age_charls + sex_charls + hukou + urbanicity_charls,data = df_charls)
charls_fgcp <- create_forest_plot_charls(model1_fgcp, " ")
print(charls_fgcp)
summary(model1_fgcp)

model1_fexf <- lm(fexf_charls ~ mobility_status_charls + age_charls + sex_charls + hukou + urbanicity_charls,data = df_charls)
charls_fexf <- create_forest_plot_charls(model1_fexf, " ")
print(charls_fexf)

model1_flang <- lm(flang_charls ~ mobility_status_charls + age_charls + sex_charls + hukou + urbanicity_charls,data = df_charls)
charls_flang <- create_forest_plot_charls(model1_flang, " ")
print(charls_flang)

model1_forient <- lm(forient_charls ~ mobility_status_charls + age_charls + sex_charls + hukou + urbanicity_charls,data = df_charls)
charls_forient <- create_forest_plot_charls(model1_forient, " ")
print(charls_forient)

model1_fmem <- lm(fmem_charls ~ mobility_status_charls + age_charls + sex_charls + hukou + urbanicity_charls,data = df_charls)
...
create_forest_plot_elsa <- function(model, title) {
  ...
  mobility_terms <- rownames(coef_table)[grep("^mobility_status_elsa", rownames(coef_table))]
  ...
  forest_data$Label <- recode(forest_data$Term,
                              "mobility_status_elsaDownwardly mobile" = "Downwardly mobile",
                              "mobility_status_elsaStably middle" = "Stably middle",
                              "mobility_status_elsaUpwardly mobile" = "Upwardly mobile",
                              "mobility_status_elsaStably high" = "Stably high",
                              "Stably low (ref)" = "Stably low (ref)"  
  )
  forest_data$Label <- factor(forest_data$Label, levels = c("Stably low (ref)", "Downwardly mobile", "Stably middle", "Upwardly mobile", "Stably high"))
...
model1_fgcp <- lm(fgcp_elsa ~ mobility_status_elsa + age_elsa + sex_elsa + race,data = df_elsa)
elsa_fgcp <- create_forest_plot_elsa(model1_fgcp, " ")
print(elsa_fgcp)
summary(model1_fgcp)

model1_fexf <- lm(fexf_elsa ~ mobility_status_elsa + age_elsa + sex_elsa + race, data = df_elsa)
elsa_fexf <- create_forest_plot_elsa(model1_fexf, " ")
print(elsa_fexf)

model1_flang <- lm(flang_elsa ~ mobility_status_elsa + age_elsa + sex_elsa + race, data = df_elsa)
elsa_flang <- create_forest_plot_elsa(model1_flang, " ")
print(elsa_flang)

model1_forient <- lm(forient_elsa ~ mobility_status_elsa + age_elsa + sex_elsa + race, data = df_elsa)
elsa_forient <- create_forest_plot_elsa(model1_forient, " ")
print(elsa_forient)

model1_fmem <- lm(fmem_elsa ~ mobility_status_elsa + age_elsa + sex_elsa + race, data = df_elsa)
...
create_forest_plot_mhas <- function(model, title) {
  ...
  mobility_terms <- rownames(coef_table)[grep("^mobility_status_mhas", rownames(coef_table))]
  ...
  forest_data$Label <- recode(forest_data$Term,
                              "mobility_status_mhasDownwardly mobile" = "Downwardly mobile",
                              "mobility_status_mhasStably middle" = "Stably middle",
                              "mobility_status_mhasUpwardly mobile" = "Upwardly mobile",
                              "mobility_status_mhasStably high" = "Stably high",
                              "Stably Low (ref)" = "Stably low (ref)",
                              "Stably low (ref)" = "Stably low (ref)"  
  )
  forest_data$Label <- factor(forest_data$Label, levels = c("Stably low (ref)", "Downwardly mobile", "Stably middle", "Upwardly mobile", "Stably high"))
...
model1_fgcp <- lm(fgcp_mhas ~ mobility_status_mhas + age_mhas + sex_mhas + urbanicity_mhas,data = df_mhas)
mhas_fgcp <- create_forest_plot_mhas(model1_fgcp, " ")
print(mhas_fgcp)
summary(model1_fgcp)

model1_fexf <- lm(fexf_mhas ~ mobility_status_mhas + age_mhas + sex_mhas + urbanicity_mhas, data = df_mhas)
mhas_fexf <- create_forest_plot_mhas(model1_fexf, " ")
print(mhas_fexf)

model1_flang <- lm(flang_mhas ~ mobility_status_mhas + age_mhas + sex_mhas + urbanicity_mhas, data = df_mhas)
mhas_flang <- create_forest_plot_mhas(model1_flang, " ")
print(mhas_flang)

model1_forient <- lm(forient_mhas ~ mobility_status_mhas + age_mhas + sex_mhas + urbanicity_mhas, data = df_mhas)
mhas_forient <- create_forest_plot_mhas(model1_forient, " ")
print(mhas_forient)

model1_fmem <- lm(fmem_mhas ~ mobility_status_mhas + age_mhas + sex_mhas + urbanicity_mhas, data = df_mhas)
...
create_forest_plot_lasi <- function(model, title) {
  ...
  mobility_terms <- rownames(coef_table)[grep("^mobility_status_lasi", rownames(coef_table))]
  ...
  forest_data$Label <- recode(forest_data$Term,
                              "mobility_status_lasiDownwardly mobile" = "Downwardly mobile",
                              "mobility_status_lasiStably middle" = "Stably middle",
                              "mobility_status_lasiUpwardly mobile" = "Upwardly mobile",
                              "mobility_status_lasiStably high" = "Stably high",
                              "Stably Low (ref)" = "Stably low (ref)",
                              "Stably low (ref)" = "Stably low (ref)"  
  )
  forest_data$Label <- factor(forest_data$Label, levels = c("Stably low (ref)", "Downwardly mobile", "Stably middle", "Upwardly mobile", "Stably high"))
...
model1_fgcp <- lm(fgcp_lasi ~ mobility_status_lasi + age_lasi + sex_lasi+ caste + urbanicity_lasi,data = df_lasi)
lasi_fgcp <- create_forest_plot_lasi(model1_fgcp, " ")
print(lasi_fgcp)
summary(model1_fgcp)

model1_fexf <- lm(fexf_lasi ~ mobility_status_lasi + age_lasi + sex_lasi+  caste + urbanicity_lasi, data = df_lasi)
lasi_fexf <- create_forest_plot_lasi(model1_fexf, " ")
print(lasi_fexf)

model1_flang <- lm(flang_lasi ~ mobility_status_lasi + age_lasi + sex_lasi+  caste + urbanicity_lasi, data = df_lasi)
lasi_flang <- create_forest_plot_lasi(model1_flang, " ")
print(lasi_flang)

model1_forient <- lm(forient_lasi ~ mobility_status_lasi + age_lasi + sex_lasi+  caste + urbanicity_lasi, data = df_lasi)
lasi_forient <- create_forest_plot_lasi(model1_forient, " ")
print(lasi_forient)

model1_fmem <- lm(fmem_lasi ~ mobility_status_lasi + age_lasi + caste + sex_lasi+  urbanicity_lasi, data = df_lasi)
lasi_fmem <- create_forest_plot_lasi(model1_fmem, " ")
print(lasi_fmem)
tab_model(model1_fgcp,model1_fexf, model1_flang,model1_forient,model1_fmem)

# jpeg("D:\\R project\\Cognition\\linear regression_percentile rank of education.jpg",res=250,w=6000,h=7000)
...
model1_fgcp_hrs_men <- lm(fgcp_hrs ~ mobility_status_hrs + age_hrs   + Race + urbanicity_hrs, data = df_hrs_men)
hrs_fgcp_men <- create_forest_plot_hrs(model1_fgcp_hrs_men, " ")
print(hrs_fgcp_men)
summary(model1_fgcp_hrs_men)

model1_fexf_hrs_men <- lm(fexf_hrs ~ mobility_status_hrs + age_hrs   + Race + urbanicity_hrs, data = df_hrs_men)
hrs_fexf_men <- create_forest_plot_hrs(model1_fexf_hrs_men, " ")
print(hrs_fexf_men)

model1_flang_hrs_men <- lm(flang_hrs ~ mobility_status_hrs + age_hrs   + Race + urbanicity_hrs, data = df_hrs_men)
hrs_flang_men <- create_forest_plot_hrs(model1_flang_hrs_men, " ")
print(hrs_flang_men)

model1_forient_hrs_men <- lm(forient_hrs ~ mobility_status_hrs + age_hrs   + Race + urbanicity_hrs, data = df_hrs_men)
hrs_forient_men <- create_forest_plot_hrs(model1_forient_hrs_men, " ")
print(hrs_forient_men)

model1_fmem_hrs_men <- lm(fmem_hrs ~ mobility_status_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs_men)
hrs_fmem_men <- create_forest_plot_hrs(model1_fmem_hrs_men, " ")
print(hrs_fmem_men)
tab_model(model1_fgcp_hrs_men,model1_fexf_hrs_men,model1_flang_hrs_men,model1_forient_hrs_men,model1_fmem_hrs_men)

#### ANALYSIS FOR ELSA ####

model1_fgcp_elsa_men <- lm(fgcp_elsa ~ mobility_status_elsa + age_elsa   + race, data = df_elsa_men)
elsa_fgcp_men <- create_forest_plot_elsa(model1_fgcp_elsa_men, " ")
print(elsa_fgcp_men)
summary(model1_fgcp_elsa_men)

model1_fexf_elsa_men <- lm(fexf_elsa ~ mobility_status_elsa + age_elsa   + race, data = df_elsa_men)
elsa_fexf_men <- create_forest_plot_elsa(model1_fexf_elsa_men, " ")
print(elsa_fexf_men)

model1_flang_elsa_men <- lm(flang_elsa ~ mobility_status_elsa + age_elsa + race, data = df_elsa_men)
elsa_flang_men <- create_forest_plot_elsa(model1_flang_elsa_men, " ")
print(elsa_flang_men)

model1_forient_elsa_men <- lm(forient_elsa ~ mobility_status_elsa + age_elsa + race, data = df_elsa_men)
elsa_forient_men <- create_forest_plot_elsa(model1_forient_elsa_men, " ")
print(elsa_forient_men)

model1_fmem_elsa_men <- lm(fmem_elsa ~ mobility_status_elsa + age_elsa  + race, data = df_elsa_men)
...
model1_fgcp_mhas_men <- lm(fgcp_mhas ~ mobility_status_mhas + age_mhas + urbanicity_mhas, data = df_mhas_men)
mhas_fgcp_men <- create_forest_plot_mhas(model1_fgcp_mhas_men, " ")
print(mhas_fgcp_men)
summary(model1_fgcp_mhas_men)

model1_fexf_mhas_men <- lm(fexf_mhas ~ mobility_status_mhas + age_mhas  + urbanicity_mhas , data = df_mhas_men)
mhas_fexf_men <- create_forest_plot_mhas(model1_fexf_mhas_men, " ")
print(mhas_fexf_men)

model1_flang_mhas_men <- lm(flang_mhas ~ mobility_status_mhas + age_mhas + urbanicity_mhas, data = df_mhas_men)
mhas_flang_men <- create_forest_plot_mhas(model1_flang_mhas_men, " ")
print(mhas_flang_men)

model1_forient_mhas_men <- lm(forient_mhas ~ mobility_status_mhas + age_mhas  + urbanicity_mhas, data = df_mhas_men)
mhas_forient_men <- create_forest_plot_mhas(model1_forient_mhas_men, " ")
print(mhas_forient_men)

model1_fmem_mhas_men <- lm(fmem_mhas ~ mobility_status_mhas + age_mhas  + urbanicity_mhas, data = df_mhas_men)
...
model1_fgcp_charls_men <- lm(fgcp_charls ~ mobility_status_charls + age_charls + urbanicity_charls + hukou, data = df_charls_men)
charls_fgcp_men <- create_forest_plot_charls(model1_fgcp_charls_men, " ")
print(charls_fgcp_men)
summary(model1_fgcp_charls_men)

model1_fexf_charls_men <- lm(fexf_charls ~ mobility_status_charls + age_charls  + urbanicity_charls  + hukou, data = df_charls_men)
charls_fexf_men <- create_forest_plot_charls(model1_fexf_charls_men, " ")
print(charls_fexf_men)

model1_flang_charls_men <- lm(flang_charls ~ mobility_status_charls + age_charls + urbanicity_charls  + hukou, data = df_charls_men)
charls_flang_men <- create_forest_plot_charls(model1_flang_charls_men, " ")
print(charls_flang_men)

model1_forient_charls_men <- lm(forient_charls ~ mobility_status_charls + age_charls  + urbanicity_charls  + hukou, data = df_charls_men)
charls_forient_men <- create_forest_plot_charls(model1_forient_charls_men, " ")
print(charls_forient_men)

model1_fmem_charls_men <- lm(fmem_charls ~ mobility_status_charls + age_charls  + urbanicity_charls  + hukou, data = df_charls_men)
...
model1_fgcp_lasi_men <- lm(fgcp_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_men)
lasi_fgcp_men <- create_forest_plot_lasi(model1_fgcp_lasi_men, " ")
print(lasi_fgcp_men)
summary(model1_fgcp_lasi_men)

model1_fexf_lasi_men <- lm(fexf_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_men)
lasi_fexf_men <- create_forest_plot_lasi(model1_fexf_lasi_men, " ")
print(lasi_fexf_men)

model1_flang_lasi_men <- lm(flang_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_men)
lasi_flang_men <- create_forest_plot_lasi(model1_flang_lasi_men, " ")
print(lasi_flang_men)

model1_forient_lasi_men <- lm(forient_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_men)
lasi_forient_men <- create_forest_plot_lasi(model1_forient_lasi_men, " ")
print(lasi_forient_men)

model1_fmem_lasi_men <- lm(fmem_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_men)
...
#### EXPORT PLOTS ####
# jpeg("D:\\R project\\Cognition\\linear regression_percentile rank of education_men.jpg", res = 250, w = 6000, h = 7000)
...
model1_fgcp_hrs_women <- lm(fgcp_hrs ~ mobility_status_hrs + age_hrs   + Race + urbanicity_hrs, data = df_hrs_women)
hrs_fgcp_women <- create_forest_plot_hrs(model1_fgcp_hrs_women, " ")
print(hrs_fgcp_women)
summary(model1_fgcp_hrs_women)

model1_fexf_hrs_women <- lm(fexf_hrs ~ mobility_status_hrs + age_hrs   + Race + urbanicity_hrs, data = df_hrs_women)
hrs_fexf_women <- create_forest_plot_hrs(model1_fexf_hrs_women, " ")
print(hrs_fexf_women)

model1_flang_hrs_women <- lm(flang_hrs ~ mobility_status_hrs + age_hrs   + Race + urbanicity_hrs, data = df_hrs_women)
hrs_flang_women <- create_forest_plot_hrs(model1_flang_hrs_women, " ")
print(hrs_flang_women)

model1_forient_hrs_women <- lm(forient_hrs ~ mobility_status_hrs + age_hrs   + Race + urbanicity_hrs, data = df_hrs_women)
hrs_forient_women <- create_forest_plot_hrs(model1_forient_hrs_women, " ")
print(hrs_forient_women)

model1_fmem_hrs_women <- lm(fmem_hrs ~ mobility_status_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs_women)
...
model1_fgcp_elsa_women <- lm(fgcp_elsa ~ mobility_status_elsa + age_elsa   + race, data = df_elsa_women)
elsa_fgcp_women <- create_forest_plot_elsa(model1_fgcp_elsa_women, " ")
print(elsa_fgcp_women)
summary(model1_fgcp_elsa_women)

model1_fexf_elsa_women <- lm(fexf_elsa ~ mobility_status_elsa + age_elsa   + race, data = df_elsa_women)
elsa_fexf_women <- create_forest_plot_elsa(model1_fexf_elsa_women, " ")
print(elsa_fexf_women)

model1_flang_elsa_women <- lm(flang_elsa ~ mobility_status_elsa + age_elsa + race, data = df_elsa_women)
elsa_flang_women <- create_forest_plot_elsa(model1_flang_elsa_women, " ")
print(elsa_flang_women)

model1_forient_elsa_women <- lm(forient_elsa ~ mobility_status_elsa + age_elsa + race, data = df_elsa_women)
elsa_forient_women <- create_forest_plot_elsa(model1_forient_elsa_women, " ")
print(elsa_forient_women)

model1_fmem_elsa_women <- lm(fmem_elsa ~ mobility_status_elsa + age_elsa  + race, data = df_elsa_women)
...
model1_fgcp_mhas_women <- lm(fgcp_mhas ~ mobility_status_mhas + age_mhas + urbanicity_mhas, data = df_mhas_women)
mhas_fgcp_women <- create_forest_plot_mhas(model1_fgcp_mhas_women, " ")
print(mhas_fgcp_women)
summary(model1_fgcp_mhas_women)

model1_fexf_mhas_women <- lm(fexf_mhas ~ mobility_status_mhas + age_mhas  + urbanicity_mhas, data = df_mhas_women)
mhas_fexf_women <- create_forest_plot_mhas(model1_fexf_mhas_women, " ")
print(mhas_fexf_women)

model1_flang_mhas_women <- lm(flang_mhas ~ mobility_status_mhas + age_mhas + urbanicity_mhas, data = df_mhas_women)
mhas_flang_women <- create_forest_plot_mhas(model1_flang_mhas_women, " ")
print(mhas_flang_women)

model1_forient_mhas_women <- lm(forient_mhas ~ mobility_status_mhas + age_mhas  + urbanicity_mhas, data = df_mhas_women)
mhas_forient_women <- create_forest_plot_mhas(model1_forient_mhas_women, " ")
print(mhas_forient_women)

model1_fmem_mhas_women <- lm(fmem_mhas ~ mobility_status_mhas + age_mhas  + urbanicity_mhas, data = df_mhas_women)
...
model1_fgcp_charls_women <- lm(fgcp_charls ~ mobility_status_charls + age_charls + urbanicity_charls + hukou, data = df_charls_women)
...
model1_fexf_charls_women <- lm(fexf_charls ~ mobility_status_charls + age_charls  + urbanicity_charls  + hukou, data = df_charls_women)
charls_fexf_women <- create_forest_plot_charls(model1_fexf_charls_women, " ")
print(charls_fexf_women)

model1_flang_charls_women <- lm(flang_charls ~ mobility_status_charls + age_charls + urbanicity_charls  + hukou, data = df_charls_women)
charls_flang_women <- create_forest_plot_charls(model1_flang_charls_women, " ")
print(charls_flang_women)

model1_forient_charls_women <- lm(forient_charls ~ mobility_status_charls + age_charls  + urbanicity_charls  + hukou, data = df_charls_women)
charls_forient_women <- create_forest_plot_charls(model1_forient_charls_women, " ")
print(charls_forient_women)

model1_fmem_charls_women <- lm(fmem_charls ~ mobility_status_charls + age_charls  + urbanicity_charls  + hukou, data = df_charls_women)
...
model1_fgcp_lasi_women <- lm(fgcp_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_women)
lasi_fgcp_women <- create_forest_plot_lasi(model1_fgcp_lasi_women, " ")
print(lasi_fgcp_women)
summary(model1_fgcp_lasi_women)

model1_fexf_lasi_women <- lm(fexf_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_women)
lasi_fexf_women <- create_forest_plot_lasi(model1_fexf_lasi_women, " ")
print(lasi_fexf_women)

model1_flang_lasi_women <- lm(flang_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_women)
lasi_flang_women <- create_forest_plot_lasi(model1_flang_lasi_women, " ")
print(lasi_flang_women)

model1_forient_lasi_women <- lm(forient_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_women)
lasi_forient_women <- create_forest_plot_lasi(model1_forient_lasi_women, " ")
print(lasi_forient_women)

model1_fmem_lasi_women <- lm(fmem_lasi ~ mobility_status_lasi + age_lasi + caste + urbanicity_lasi, data = df_lasi_women)
...
#### EXPORT PLOTS ####
# jpeg("D:\\R project\\Cognition\\linear regression_percentile rank of education_women.jpg", res = 250, w = 6000, h = 7000)
...
jpeg("D:\\R project\\Cognition\\lr_cognition_PR.jpg",res=320,w=8800,h=4800)
hrs_fgcp + elsa_fgcp + mhas_fgcp + charls_fgcp + lasi_fgcp + hrs_fgcp_men + elsa_fgcp_men + mhas_fgcp_men + charls_fgcp_men + lasi_fgcp_men + hrs_fgcp_women + elsa_fgcp_women + mhas_fgcp_women + charls_fgcp_women + lasi_fgcp_women +
  plot_layout(ncol =  5)
dev.off()

#2.executive function-overall/men/women
jpeg("D:\\R project\\Cognition\\lr_exf_PR.jpg",res=320,w=8800,h=4800)
hrs_fexf + elsa_fexf + mhas_fexf + charls_fexf + lasi_fexf + hrs_fexf_men + elsa_fexf_men + mhas_fexf_men + charls_fexf_men + lasi_fexf_men + hrs_fexf_women + elsa_fexf_women + mhas_fexf_women + charls_fexf_women + lasi_fexf_women +
  plot_layout(ncol =  5)
dev.off()

#3.language-overall/men/women
jpeg("D:\\R project\\Cognition\\lr_lang_PR.jpg",res=320,w=8800,h=4800)
hrs_flang + elsa_flang + mhas_flang + charls_flang + lasi_flang + hrs_flang_men + elsa_flang_men + mhas_flang_men + charls_flang_men + lasi_flang_men + hrs_flang_women + elsa_flang_women + mhas_flang_women + charls_flang_women + lasi_flang_women +
  plot_layout(ncol =  5)
dev.off()

#4.orientation-overall/men/women
jpeg("D:\\R project\\Cognition\\lr_ori_PR.jpg",res=320,w=8800,h=4800)
hrs_forient + elsa_forient + mhas_forient + charls_forient + lasi_forient + hrs_forient_men + elsa_forient_men + mhas_forient_men + charls_forient_men + lasi_forient_men + hrs_forient_women + elsa_forient_women + mhas_forient_women + charls_forient_women + lasi_forient_women +
  plot_layout(ncol =  5)
dev.off()

#5.memory-overall/men/women
jpeg("D:\\R project\\Cognition\\lr_mem_PR.jpg",res=320,w=8800,h=4800)
hrs_fmem + elsa_fmem + mhas_fmem + charls_fmem + lasi_fmem + hrs_fmem_men + elsa_fmem_men + mhas_fmem_men + charls_fmem_men + lasi_fmem_men + hrs_fmem_women + elsa_fmem_women + mhas_fmem_women + charls_fmem_women + lasi_fmem_women +
  plot_layout(ncol =  5)
dev.off()
...
#pr
#hrs
model1_fgcp <- lm(fgcp_hrs ~ mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
summary(model1_fgcp)

coef_table <- summary(model1_fgcp)$coefficients
conf_int <- confint(model1_fgcp)

model1_fexf <- lm(fexf_hrs ~ mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
model1_flang <- lm(flang_hrs ~ mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
model1_forient <- lm(forient_hrs ~ mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
model1_fmem <- lm(fmem_hrs ~ mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
tab_model(model1_fgcp, model1_fexf,model1_flang,model1_forient,model1_fmem)
...
#charls
model1_fgcp <- lm(fgcp_charls ~ mobility_status_charls * sex_charls + age_charls  +hukou + urbanicity_charls, data = df_charls)
summary(model1_fgcp)

coef_table <- summary(model1_fgcp)$coefficients
conf_int <- confint(model1_fgcp)

model1_fexf <- lm(fexf_charls ~ mobility_status_charls * sex_charls + age_charls  + hukou + urbanicity_charls, data = df_charls)
model1_flang <- lm(flang_charls ~ mobility_status_charls * sex_charls + age_charls  + hukou + urbanicity_charls, data = df_charls)
model1_forient <- lm(forient_charls ~ mobility_status_charls * sex_charls + age_charls  + hukou + urbanicity_charls, data = df_charls)
model1_fmem <- lm(fmem_charls ~ mobility_status_charls * sex_charls + age_charls  + hukou + urbanicity_charls, data = df_charls)
tab_model(model1_fgcp, model1_fexf,model1_flang,model1_forient,model1_fmem)
...
#harmonized
#hrs
model1_fgcp <- lm(fgcp_hrs ~ H_mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
summary(model1_fgcp)

coef_table <- summary(model1_fgcp)$coefficients
conf_int <- confint(model1_fgcp)

model1_fexf <- lm(fexf_hrs ~ H_mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
model1_flang <- lm(flang_hrs ~ H_mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
model1_forient <- lm(forient_hrs ~ H_mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
model1_fmem <- lm(fmem_hrs ~ H_mobility_status_hrs * sex_hrs + age_hrs  + Race + urbanicity_hrs, data = df_hrs)
tab_model(model1_fgcp, model1_fexf, model1_flang, model1_forient, model1_fmem)
...
contingency_table <- table(
  df_charls$mobility_status_charls,
...
create_forest_plot_hrs_combined <- function(model_men, model_women, title) {
  ...
  mobility_terms_men <- rownames(coef_table_men)[grep("^mobility_status_hrs", rownames(coef_table_men))]
  ...
  mobility_terms_women <- rownames(coef_table_women)[grep("^mobility_status_hrs", rownames(coef_table_women))]
...
create_forest_plot_elsa_combined <- function(model_men, model_women, title) {
  coef_table_men <- summary(model_men)$coefficients
  conf_int_men <- confint(model_men)
  mobility_terms_men <- rownames(coef_table_men)[grep("^mobility_status_elsa", rownames(coef_table_men))]
  ...
  coef_table_women <- summary(model_women)$coefficients
  conf_int_women <- confint(model_women)
  mobility_terms_women <- rownames(coef_table_women)[grep("^mobility_status_elsa", rownames(coef_table_women))]
...
create_forest_plot_mhas_combined <- function(model_men, model_women, title) {
  coef_table_men <- summary(model_men)$coefficients
  conf_int_men <- confint(model_men)
  mobility_terms_men <- rownames(coef_table_men)[grep("^mobility_status_mhas", rownames(coef_table_men))]
  ...
  coef_table_women <- summary(model_women)$coefficients
  conf_int_women <- confint(model_women)
  mobility_terms_women <- rownames(coef_table_women)[grep("^mobility_status_mhas", rownames(coef_table_women))]
...
create_forest_plot_charls_combined <- function(model_men, model_women, title) {
  coef_table_men <- summary(model_men)$coefficients
  conf_int_men <- confint(model_men)
  mobility_terms_men <- rownames(coef_table_men)[grep("^mobility_status_charls", rownames(coef_table_men))]
  ...
  coef_table_women <- summary(model_women)$coefficients
  conf_int_women <- confint(model_women)
  mobility_terms_women <- rownames(coef_table_women)[grep("^mobility_status_charls", rownames(coef_table_women))]
...
create_forest_plot_lasi_combined <- function(model_men, model_women, title) {
  coef_table_men <- summary(model_men)$coefficients
  conf_int_men <- confint(model_men)
  mobility_terms_men <- rownames(coef_table_men)[grep("^mobility_status_lasi", rownames(coef_table_men))]
  ...
  coef_table_women <- summary(model_women)$coefficients
  conf_int_women <- confint(model_women)
  mobility_terms_women <- rownames(coef_table_women)[grep("^mobility_status_lasi", rownames(coef_table_women))]
...
jpeg("D:\\R project\\Cognition\\lr_cognition_PR_0219.jpg", res=300, w=9000, h=5600)
